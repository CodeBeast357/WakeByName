...
{
    if (!LoadString(nID)) { // Error : string not found !
...
{                               // uses FormatMessage() arguments notation : %1 %2 ...
    ...
    CString strFormat;
    if (!strFormat.LoadString(nFormatID)) { // Format string not found -> Load error string : ???
        TRACE(_T("Unknown resource string id : %d\n"), nFormatID);
        ASSERT(false);
        CString::operator=(_T("???")); // In case of error, set string contents to ???. Compatible with VC6 and VC7.
    } else { // OK, format string was loaded successfully
